...
options. AGMG is used with default parameters.
...
to note that all the tests use convex polygonal cells because of the implementation of
the sweeping algorithm. In \cite{pwld_3}, the PWLD discretization was shown to
give accurate results even for non-convex polygonal cells.
...
relative tolerance used by the Source Iteration solver is compared to
$\frac{\|\Phi^{(l+1)}-\Phi^{(l)}\|_2}{\|\Phi^{(l+1)}\|_2}$. When a Krylov solver
is used, the relative tolerance is compared to
$\frac{\|\textrm{residual}\|2}{\|\textrm{right hand side}\|_2}$.
...
%
The different linear solvers are compared for the polygonal grid in Table~\ref
{comparison_homog_poly_si}. In Table~\ref{comparison_homog_poly_gmres}, the SI
solver is replaced by GMRes, {\it GMRes iterations} is the number of iterations
needed to solve the problem.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this test, we show that MIP and PWLD are efficient even highly degenerated
polygons. The domain is $100cm \times 100cm$, $\Sigma_t = 1cm^{-1}$ and
$\Sigma_s=0.99999cm^{-1}$. There is an isotropic incoming flux of intensity
$5cm^{-2}s^{-1}$ on the bottom boundary, vacuum on the top boundary, and the
left and right boundaries are reflective. There is no source and we use an $S_8$
...
The mesh is composed of 10 rectangular cells: the first cell has 5
sides, the second cell has 7 sides, the third cell has 9 sides, the fourth cell
has 11 sides, the fifth cell has 13 sides, the sixth cell has 15 sides, the
seventh cell has 17 sides, the eighth cell has 19 sides, the ninth cell has 21
sides, and the tenth cell has 23 sides. For a total of 140 degrees of freedom. 
Part of the mesh is shown in Figure~\ref {fig_degenerated_polygons}.
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.2\textwidth]{degenerated_polygons}
  \caption{Mesh of degenerated polygons.}
  \label{fig_degenerated_polygons}
\end{figure}
Without DSA, SI needs 17592 iterations to converge whereas it takes only 20
iterations when MIP is used. As expected for this problem, the scalar flux
decreases linearly see Figure~\ref {fig_no_dsa}.
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.3\textwidth]{no_dsa}
