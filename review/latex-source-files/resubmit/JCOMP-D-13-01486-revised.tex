...
\ead{jean.ragusa@tamu.edu} % ORCID: 0000-0001-9870-6447
...
\subsubsection{Test Case with Degenerated Polygons}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this test, we show that MIP-DSA discretized with PWLD is efficient, even for highly degenerated
polygons. A source-free homogeneous problem ($\Sigma_t = 1cm^{-1}$ and
$\Sigma_s=0.999\,99cm^{-1}$) of dimension $100cm \times 100cm$ is used. An isotropic incoming flux of intensity
$5cm^{-2}ster^{-1}s^{-1}$ is applied on the bottom boundary; a vacuum boundary condition is set on the top; the
left and right boundaries are reflective. We use an $S_8$
angular quadrature. A relative tolerance of $10^{-6}$ is used for the Source
Iteration solver while the MIP-DSA solvers employs a relative of $10^{-8}$.
The mesh is composed of 10 rectangular shaped cells: the first cell has 5
vertices, the second cell has 7, and so on and so forth until the tenth cell which has 23 vertices. 
There is a total of 140 spatial degrees of freedom. 
The mesh is shown in Figure~\ref {fig_degenerated_polygons}.
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{degenerated_polygons_v2}
  \caption{Grid composed of degenerated polygons.}
  \label{fig_degenerated_polygons}
\end{figure}
Without DSA, SI needs 17,592 iterations to converge whereas it takes only 20
iterations when MIP-DSA is employed. The scalar flux solution is shown on 
Figure~\ref {fig_no_dsa} and the contour lines of the scalar flux are shown on Figure~\ref
{fig_contour}.
\begin{figure}[!htbp]
  \begin{subfigure}{0.45\textwidth}
    \centering
...
\bibitem{starccm}
{StarCCM+}: Polyhedral mesh generation,
  {http://www.cd-adapco.com/cfdImage/polyhedral-mesh-generation}, accessed on
  May 7, 2014.
...
\bibitem{m4s}
E.~W. Larsen.
\newblock {Unconditionally Stable Diffusion-Synthetic Acceleration Methods for
  Slab Geometry Discrete Ordinates Equations. Part I: Theory}.
\newblock {\em Nuclear Science and Engineering}, 82, 1982.
...
\bibitem{cfm_dfm}
James~S. Warsa.
\newblock {A Continuous Finite Element-Based, Discontinuous Finite Element
  Method for Sn Transport}.
\newblock {\em Nuclear Science and Engineering}, 160:385--400, 2008.

\bibitem{wla_pwl}
Anthony~P. Barbu and Marvin~L. Adams.
\newblock {Semi-Consistent Diffusion Synthetic Acceleration for Discontinuous
  Discretizations of Transport Problems}.
\newblock {\em International Conference on Mathematics, Computational Methods
...
\newblock Second edition, 2000.

\bibitem{amg_course}
Christian Wagner.
\newblock {\em {Introduction to Algebraic Multigrid}}.
\newblock 1998.
\newblock {Course Notes of an Algebraic Multigrid Course at the University of
...
\newblock Technical Report SAND2006-2649, Sandiad National Laboratories, 2006.

\bibitem{agmg_guide}
Yvan Notay.
\newblock {User's Guide to AGMG}.
\newblock Technical report, Universite Libre de Bruxelles, 2011.

\bibitem{agmg}
Y.~Notay.
\newblock {An aggregation-based algebraic multigrid method}.
\newblock {\em Electronic Transactions on Numerical Analysis}, 37:123--146,
  2010.

\bibitem{agmg2}
Artem Napov and Yvan Notay.
\newblock {An algebraic multigrid method with guaranteed convergence rate}.
\newblock {\em SIAM Journal of Scientific Computing}, 34(2):1079--1109, 2012.

\bibitem{agmg3}
Yvan Notay.
\newblock {Aggregation-based algebraic multigrid for convection-diffusion
  equations}.
\newblock {\em SIAM Journal of Scientific Computing}, 34(4):2288--2316, 2012.

\bibitem{dgfem}
Todd~A. Wareing, John~M. McGhee, Jim~E. Morel, and Shawn~D. Pautz.
\newblock {Discontinuous Finite Element $S_N$ Methods on Three-Dimensional
  Unstructured Grids}.
\newblock {\em Nuclear Science and Engineering}, 138:256--268, January 2011.

\bibitem{pwld_2d}
Hiromi~G. Stone and Marvin~L. Adams.
\newblock {A piecewise linear finite element basis with application to particle
  transport}.
\newblock {\em Nuclear Mathematical and Computational Sciences : A Century in
  Review, A Century Anew, Gatlinburg, Tennessee}, April 2003.

\bibitem{angular_multigrid_1d}
J.E. Morel and T.A. Manteuffel.
\newblock {An Angular Multigrid Acceleration Technique for $S_n$ Equations with
  Highly Forward-Peaked Scattering}.
\newblock {\em Nuclear Science and Engineering}, 107:330--342, 1991.

\bibitem{angular_multigrid_bruno}
Bruno Turcksin, Jean~C. Ragusa, and Jim~E. Morel.
\newblock {Angular Multigrid Preconditioner for Krylov-based Solution
  Techniques applied to the Sn Equations with Highly Forward-Peaked
  Scattering}.
\newblock {\em Transp. Theor. \& Stat. Phys.}, 41:1--22, 2012.

\bibitem{Kanschat2007}
G.~Kanschat.
\newblock {\em Discontinuous Galerkin Methods for Viscous Incompressible Flow}.
\newblock Advances in Numerical Mathematics. Teubner Research, Wiesbaden,
  Germany, 2007.
...
\bibitem{arnold2000discontinuous}
Douglas~N Arnold, Franco Brezzi, Bernardo Cockburn, and Donatella Marini.
\newblock Discontinuous galerkin methods for elliptic problems.
\newblock In {\em Discontinuous Galerkin Methods}, pages 89--101. Springer,
  2000.

\bibitem{wang_personal_comm}
Yaqi Wang.
\newblock Personal E-mail communication (2012-08-29).

\bibitem{textbook_eff}
James~L. Thomas, Boris Diskin, and Achi Brandt.
\newblock {Textbook Multigrid Efficiency for Fluid Simulations}.
\newblock {\em Annual Review of Fluid Mechanics}, 35:317--340, January 2003.

\bibitem{k_cycle}
Yvan Notay and Panayot~S. Vassilevski.
\newblock {Recursive Krylov-based multigrid cycles}.
\newblock {\em Numerical Linear Algebra with Applications}, 15:473--487, 2008.

\bibitem{mis}
Ray~S. Tuminaro and Charles Tong.
\newblock {Parallel Smoothed Aggregation Multigrid: Aggregation Strategies on
  Massively Parallel Machines}.
\newblock {\em SC Conference}, 0:5, 2000.

\bibitem{graph_coloring}
Mark~T. Jones and Paul~E. Plassmann.
\newblock {A Parallel Graph Coloring Heuristic}.
\newblock {\em SIAM Journal on Scientific Computing}, 14:654, 1992.

\bibitem{quadrature}
Richard Sanchez and Jean~C. Ragusa.
\newblock On the construction of galerkin angular quadratures.
\newblock {\em Nuclear Science and Engineering}, 169:133--154, 2011.

\bibitem{multisweep}
Marvin~L. Adams and Todd~A. Wareing.
\newblock {Diffusion-synthetic acceleration given anisotropic scattering,
  general quadratures, and multidimensions}.
\newblock {\em Conference: American Nuclear Society(ANS) annual meeting, San
  Diego}, 68, June 1993.

\bibitem{pwld_3d}
Teresa~S. Bailey.
\newblock {\em {The piecewise linear discontinuous finite element method
  applied to the RZ and XYZ transport equations}}.
\newblock PhD thesis, Texas A\&M University, 2008.
